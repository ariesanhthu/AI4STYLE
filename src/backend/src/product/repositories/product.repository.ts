...
import { IProductRepositoryInterface, ProductJoinOptions, ProductOptionJoinOptions } from './product.repository.interface';
...
export class ProductRepository implements IProductRepositoryInterface {
  ...
  async findAllProduct(query: GetListProductDto, options?: ProductJoinOptions): Promise<ProductEntity[]> {
  ...
  async findProductById(id: string, options?: ProductJoinOptions): Promise<ProductEntity | null> {
  ...
  async updateProduct(id: string, product: Partial<ProductEntity>): Promise<ProductEntity | null> {
  ...
  async findAllProductOption(query: GetListProductClientDto, options?: ProductOptionJoinOptions): Promise<ProductOptionEntity[]> {
    const whereClause: any = {};
  ...
  async findProductOptionById(id: string, options?: ProductOptionJoinOptions): Promise<ProductOptionEntity | null> {
  ...
  async createProductOption(productOption: ProductOptionEntity): Promise<ProductOptionEntity> {
  ...
  async updateProductOption(
    ...
    const updated = await this.prismaService.productOption.update({
      ...
      data: {
        ...
        ...(productOption.colorFamily && { color_family: productOption.colorFamily }),
        ...(productOption.images && { images: productOption.images }),
        ...(productOption.price !== undefined && { price: productOption.price }),
        ...(productOption.newPrice !== undefined && { new_price: productOption.newPrice }),
        ...(productOption.outOfStock !== undefined && { out_of_stock: productOption.outOfStock }),
        ...(productOption.isShow !== undefined && { is_show: productOption.isShow }),
  ...
  async createBulkProductOptions(productOptions: ProductOptionEntity[]): Promise<ProductOptionEntity[]> {
      ...
      options.map((option) =>
        this.prismaService.productOption.update({
          ...
          data: {
            ...
            ...(option.newPrice !== undefined && { new_price: option.newPrice }),
            ...(option.outOfStock !== undefined && { out_of_stock: option.outOfStock }),
  ...
  async createBulkProductVariants(variants: ProductVariant[]): Promise<any> {
  ...
  async updateBulkProductVariants(
    variants: Array<Partial<ProductVariant> & { variant_id: string }>,
  ): Promise<any> {
    const updated = await Promise.all(
      variants.map((variant) =>
        this.prismaService.productVariant.update({
          ...
          data: {
            ...
            ...(variant.new_price !== undefined && { new_price: variant.new_price }),
  ...
  async syncProductOptionPricing(optionId: string): Promise<void> {
    ...
    const newPrices = variants.map((v) => v.new_price).filter((p): p is number => p !== null);
