...
import { CreateOrderDetailDto, CreateOrderDto, GetListOfOrdersQueryDto, UpdateOrderStatusDto } from "../dtos";
import { OrderEntity, OrderDetailEntity } from "../entities";
...
export class OrderService {
  ...
  async createOrder(userId: string, orderData: CreateOrderDto) {
    try {
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        const enhancedDetails: (CreateOrderDetailDto & { pricePerUnit: number })[] = [];
        for (const detail of orderData.orderDetails) {
          const variant = variants.find(v => v.variant_id === detail.variantId);
          if (!variant) {
            throw new BadRequestException(`Variant ${detail.variantId} not found`);
          }
        ...
        const orderDetails = enhancedDetails.map(detail => 
        ...
        const createdOrder = await this.orderRepository.creaeteOrder(orderEntity);
  ...
  private async cancelOrder(orderId: string) {
    try {
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (order.status === EOrderStatus.DELIVERED || order.status === EOrderStatus.CANCELED) {
          throw new BadRequestException(`Order with status ${order.status} cannot be canceled`);
        }
        ...
        if (order.orderDetails && order.orderDetails.length > 0) {
          ...
          await this.productService.modifyProductVariantStock({ variants: stockUpdates });
        }
        ...
        const canceledOrder = await this.orderRepository.updateOrder(orderId, order);
   ...
   */
  private validateStatusTransition(currentStatus: EOrderStatus, newStatus: EOrderStatus) {
    const validTransitions: Record<EOrderStatus, EOrderStatus[]> = {
      [EOrderStatus.PENDING_PAYMENT]: [EOrderStatus.PENDING, EOrderStatus.CANCELED],
