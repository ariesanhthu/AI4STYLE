...
import { ApiZodBody, ApiZodErrorResponse, ApiZodQuery, ApiZodResponse, CurrentUser, Permissions, ZodBody, ZodQuery } from "../../shared/decorators";
...
export class OrderAdminController extends BaseOrderController {
  ...
  @ApiZodResponse({ status: 200, schema: createPaginationCursorResponseSchema(orderResponseSchema), description: "List of orders retrieved successfully" })
  @ApiOperation({ summary: "Get list of orders with filtering and pagination" })
  @ApiZodQuery(getListOfOrdersQuerySchema)
  @Get()
  getListOfOrders(@ZodQuery(getListOfOrdersQuerySchema) query: GetListOfOrdersQueryDto) {
    return this.orderService.getListOfOrders(query);
  }

  @ApiZodResponse({ status: 200, schema: orderResponseSchema, description: "Order status updated successfully" })
  ...
  @ApiZodResponse({ status: 200, schema: statusResponseSchema, description: "Order deleted successfully" })
