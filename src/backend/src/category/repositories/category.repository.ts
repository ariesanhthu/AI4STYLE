...
import { ICategoryRepository, CategoryJoinOptions, CategoryUniqueCheck } from "./category.repository.interface";
...
export class CategoryRepository implements ICategoryRepository {
  ...
  async findById(id: string, options?: CategoryJoinOptions): Promise<CategoryEntity | null> {
  ...
  async findBySlug(slug: string, options?: CategoryJoinOptions): Promise<CategoryEntity | null> {
    const category = await this.prisma.category.findUnique({
      where: { slug },
      include: {
        parent: options?.includeParent || false,
  ...
  async checkUnique(check: CategoryUniqueCheck): Promise<boolean> {
  ...
  async findAll(query: PaginationCursorQueryDto, options?: CategoryJoinOptions): Promise<CategoryEntity[]> {
