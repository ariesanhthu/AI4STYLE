...
import { IUserRepository, UserJoinOptions } from "./user.repository.interface";
...
export class UserRepository implements IUserRepository {
  ...
  async findById(id: string, options?: UserJoinOptions): Promise<UserEntity | null> {
  ...
  async findByEmail(email: string, options?: UserJoinOptions): Promise<UserEntity | null> {
    const user = await this.prismaService.user.findUnique({
      where: { email },
      include: options?.includeRole ? { role: true } : undefined,
    });
  ...
  async findAll(query: GetListUserDto, options?: UserJoinOptions): Promise<UserEntity[]> {
